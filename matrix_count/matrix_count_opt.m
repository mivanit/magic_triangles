...
per_finished = -1;
for p = 1:nperms
    per = floor(p/nperms*100);
    if per > per_finished
        per_finished = per;
        disp([num2str(n) ': ' num2str(per) '%']);
        toc
    end
	perm = next_permutation(perm,ncards);
%    perm = perms(p,:);
    matrix = NaN(n);
    for i = 1:n
        v = M(perm(i),:);
...
function outperm = next_permutation(inperm,ncards)
n=length(inperm);
outperm = inperm;
adding = true;
i=n;
while adding
    if outperm(i) == ncards-(n-i);
       i = i-1;
    else
        adding = false;
    end
end
outperm(i) = outperm(i) + 1;
if i ~= n
    for j = i+1:n
        outperm(j) = outperm(j-1) + 1;
    end
end
end
